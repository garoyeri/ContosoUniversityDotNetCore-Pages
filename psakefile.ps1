...
task Test -depends Compile, MigrateTest -description "Run unit tests" {
    ...
    get-childitem . *Tests -directory | foreach-object {
        exec { dotnet test --configuration $configuration --no-build -l "trx;LogFileName=$($_.name).trx" -l "html;LogFileName=$($_.name).html" -r $testResults } -workingDirectory $_.fullname
    }
}
